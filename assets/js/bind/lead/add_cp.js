...
// Assume sessionTypeId is available in the HTML page
console.log("Session Type ID:", sessionTypeId); 
if (sessionTypeId == "5" && $("[name=openlead]").val() != "") {
    $("#sidecard").addClass("open");
    getCpDetails($("[name=openlead]").val());
}
    ...
    if ($("[name=mode]").val() != "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (sessionTypeId == 4) {
                    alert("Redirecting to profile.php");
                    window.location.href =  baseurl +"/profile.php";
                    return;  // Exit the function to avoid AJAX call
                }
               
                var d = data.split('/');
                console.log("Split data:", d);
                //updatecplead
                if (d[0] == "insertconfirmcp" || d[0] == "updateconfirmcp") {
                    // alert("came");
    ...
    var $validator = $('form#customerform').validate({
        ...
        rules: {
            ...
            mobno: {
                ...
                maxlength: 10,
                remote: {
                    url: baseurl + "actions/sourceexist.php",
                    type: "post",
                    cache:false,
                    data: {
                        mode:"leadmob",
